...
class Task(BaseModel): request:str; environment:str="sandbox"; account_alias:str="demo"

def persist(x): (DATA/f"{x['id']}.json").write_text(json.dumps(x,indent=2))
@app.get("/")
...
@app.get("/api/tasks")
def tasks(): return [json.loads(p.read_text()) for p in DATA.glob("*.json")]
@app.post("/api/tasks")
def propose(t:Task):
